...
-   We can try out a common estimator used for count outcomes, the `Poisson GLM`
...
-   Alternative specification option- use a simple `log transformation` to get comparable resukt
...
**So the estimate can be interpret as follows:**
...
summ(m6_clust,
     robust = "HC2",
     cluster = "pair_id", # Add SE based on pair-clusters
     model.fit = FALSE)
...
-   Our treatment effect result is `robust` (remains stable) after adding the clustered SE adjustment
...
-   The treatment was found to reduce waste pile burning by 27% (a statistically significant result; `p<.001`).

    Model `M6` replicates the treatment effect reported in Buntaine et al., 2024 (see Figure 2; T+6)
    
------------------------------------------------------------------------
   
### Figure 2: Buntaine et al. (2024) - Replication of treatment estimate (`T+6`) 
